...
void do_up(void)
{
	...
	if (go_back_chunks(1, &openfile->current, &leftedge) > 0) {
		/*If DIRECTORY is set and we are at the topmost line, continue highlighting this line*/
		if(ISSET(DIRECTORY)) {
			shift_held = TRUE;
			openfile->mark = openfile->current;
			openfile->mark_x = openfile->current_x + strlen(openfile->current->data);
	...
	/* If DIRECTORY is set, highlight the whole line*/
	if(ISSET(DIRECTORY)) {
		shift_held = TRUE;
		openfile->mark = openfile->current;
		openfile->mark_x = openfile->current_x + strlen(openfile->current->data);
