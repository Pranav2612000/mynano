...
int parse_kbinput(WINDOW *win)
{
	...
	/* If a key not valid in DIRECTORY mode is pressed, do nothing(return error) */
	if(ISSET(DIRECTORY) && (!((keycode == '\r') || (keycode == KEY_UP) || (keycode == KEY_DOWN) || (keycode == ((int)'x' & 0x1f)) || (keycode == KEY_LEFT) || (keycode == KEY_RIGHT)))) {
		return ERR;
	}

	/* If key 'm' is pressed */
	if(keycode ==  '\r' && ISSET(DIRECTORY)) {
		char *newfile;
		/*UNSET DIRECTORY as we are not sure whether the next path is a file or folder*/
		UNSET(DIRECTORY);

		/* Add the path to be visited in newfile*/
		newfile = NULL;
		newfile = (char*)malloc(sizeof(char) * (strlen(openfile->filename) + strlen(openfile->current->data) + 1));
		strcpy(newfile, openfile->filename);
		strcat(newfile, openfile->current->data);
		
		/* Change the filename to newfile i.e. new path name */
		openfile->filename = mallocstrcpy(openfile->filename, newfile);

		/*Add the current path to path_list dir_browser before moving forward*/
		add_to_path_list();

		/*Remove the colors for this openfile*/
		syntaxes = NULL;
		...
		/* Replace the buffer to add contents of the newfile to openfile */
		replace_buffer(newfile);

		/* Set modified property of openfile to false */
		openfile->modified = FALSE;

		/* Refresh the buffer to display the newly added contents*/
		edit_refresh();
		
		/* Free the used variables */
		free(newfile);
		
		/* Return ERR as no other effect to be appled to the file */
		return ERR;
	}
	/*Move backwards in path_list dir_browser if in DIRECTORY mode*/
	if(keycode == KEY_LEFT && ISSET(DIRECTORY)) {
		move_backwards_in_path();
		return ERR;
	}
	/*Move forwards in path_list dir_browser if in DIRECTORY mode*/
	if(keycode == KEY_RIGHT && ISSET(DIRECTORY)) {
...
/*This function adds the openfile->filename to dir_browser list, moves the current pointer to the correct position, changes the list if required*/
void add_to_path_list() {

	/* If the list is empty, first element is to be added */
	if(dir_browser->head == NULL) {

		/* Make head point to the new node*/
		path_list_node *temp;
		temp = (path_list_node*)malloc(sizeof(path_list_node));
		temp->prev = temp->next = NULL;
		temp->name = NULL;
		temp->name = mallocstrcpy(temp->name, openfile->filename);
		dir_browser->head = temp;

		/*Set current pointer to the new node*/
		dir_browser->current = temp;
		return;
	} else {
		if(dir_browser->current->next == NULL) {

			/* Add the new node at the end of the list i.e. the user has not moved back/forward in the path.
			 * The current pointer points to the last node. */
			path_list_node *temp;
			temp = (path_list_node*)malloc(sizeof(path_list_node));
			temp->next = NULL;
			temp->prev = dir_browser->current;
			dir_browser->current->next = temp;
			dir_browser->current = temp;
			temp->name = NULL;
			temp->name = mallocstrcpy(temp->name, openfile->filename);
			return;
			...
			/* Add the new node somewhere in between i.e Not at the last node i.e the user moved moved forward/back in the path.
			 * The current pointer points to the last node added.
			 * All nodes after the current node are freed. */
			path_list_node *looper, *temp, *to_be_deleted;
			looper = dir_browser->current->next;

			/* Free nodes after the current node, as a new path is created. */
			while(looper != NULL) {
				free(looper->name);
				to_be_deleted = looper;
				looper = looper->next;
				free(to_be_deleted);
			}

			/* Add new node at the right position and set current at appropriate position. */
			temp = (path_list_node*)malloc(sizeof(path_list_node));
			temp->next = NULL;
			temp->prev = dir_browser->current;
			dir_browser->current->next = temp;
...
void move_backwards_in_path() {

	/* If at the first element in the path, do nothing. */
	if(dir_browser->current->prev == NULL)
		return;
	...
	/* Set newfile equal to the path to be visited (previous path). */
	newfile = NULL;
	newfile = mallocstrcpy(newfile, dir_browser->current->prev->name);

	/* Make current pointer point to the previous node. */
	dir_browser->current = dir_browser->current->prev;

	/* Set openfile name equal to newfile. */
	openfile->filename = mallocstrcpy(openfile->filename, newfile);
...
void move_forwards_in_path() {
	char *newfile;
	newfile = NULL;
	/* If the current position is not the last in the path. */
	if(dir_browser->current->next != NULL) {

		/*UNSET DIRECTORY as we are not sure whether the next path is a file or folder*/
		UNSET(DIRECTORY);

		/* Set newfile equal to the path to be visited (next path). */
		newfile = mallocstrcpy(newfile, dir_browser->current->next->name);

		/* Make current pointer point to the next node. */
		dir_browser->current = dir_browser->current->next;
