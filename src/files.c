...
void initialize_buffer_text(void)
{
	...
	if(ISSET(DIRECTORY)) {
		openfile->fileage->data = NULL;

	/*	char *strs, *strc;
		strs = NULL;
		strc = NULL;
		strs = mallocstrcpy(strs, "\"dirs\" \".*\"");*/
		//strc = mallocstrcpy(strc, "blue \".*/\"");
		//str = mallocstrcpy(str, "set smooth");
		/*parse_syntax(strs);
		parse_colors(strc, NANO_REG_EXTENDED);*/
...
bool open_buffer(const char *filename, bool new_buffer)
{
	...
	if (*filename != '\0') {
		...
		if (stat(realname, &fileinfo) == 0 && !(S_ISREG(fileinfo.st_mode) ||
			...
			/* If we are opening a directory. */
			if (S_ISDIR(fileinfo.st_mode)) {
				statusline(ALERT, _("\"%s\" is a directory"), realname);

				if(filename[strlen(filename) - 1] == '/') {
					/* If filename ends with a '/' */
					realname = mallocstrcpy(realname, filename);
					SET(DIRECTORY);
				} else {
					/* If filename does not end with a '/', add an '/' to filename. */
					char *temp;	
					temp = NULL;
					temp = (char *)malloc(sizeof(filename) + 2);
					strcpy(temp, filename);
					strcat(temp, "/");
					realname = mallocstrcpy(realname, temp);
					SET(DIRECTORY);
					free(temp);
				}

				/*Initialization of path_list dir_browser. */
				path_list_init();
	...
	/* If DIRECTORY is set, SET VIEW_MODE. */
	if(ISSET(DIRECTORY)) {
		SET(VIEW_MODE);
	}
	...
	/* If the filename isn't blank, and we are not in NOREAD_MODE, or we are opening a directory
	 * open the file.  Otherwise, treat it as a new file. */
	rc = ((ISSET(DIRECTORY) || filename[0] != '\0') && !ISSET(NOREAD_MODE)) ?
	...
	/* If DIRECTORY is set, add the current filename(Directory name) to path_list dir_browser. */
	if(ISSET(DIRECTORY)) {
		add_to_path_list();	
	}
...
void read_file(FILE *f, int fd, const char *filename, bool undoable)
{
	...
	if(ISSET(DIRECTORY)) {
		...
		/* Display the list of files and folders for the current path. */
...
int open_file(const char *filename, bool newfie, bool quiet, FILE **f)
{
	...
	/* Don't open character files, or block files. */
	if (S_ISCHR(fileinfo.st_mode) || S_ISBLK(fileinfo.st_mode)) {
		statusline(ALERT, _("\"%s\" is a device file"), filename);
		free(full_filename);
		return -1;
	}

	/* If we are opening a directory */
	if(ISSET(DIRECTORY) || S_ISDIR(fileinfo.st_mode) ) {
		SET(DIRECTORY);
		statusbar(_("Reading Directory"));
		free(full_filename);

		/* Set appropriate colors - Bright Blue for directories. More Customizations can be added */
		char *strs, *strc;
		strs = NULL;
		strc = NULL;
		strs = mallocstrcpy(strs, "\"dirs\" \".*\"");
		strc = mallocstrcpy(strc, "brightblue \".*/\"");
		parse_syntax(strs);
		parse_colors(strc, NANO_REG_EXTENDED);
		color_update();
		color_init();
	...
	/* If the file we opened is not a directory, UNSET VIEW_MODE. */
	UNSET(VIEW_MODE);
	do_rcfiles();
...
/* This function opens the directory saved in openfile->filename,
 * calls add_element to add the files/folders to the buffer,
 * and highlights the first element. */ 
void get_contents(const char *filename) {
	DIR *current_dir;
	struct dirent *element;
	char *element_name;
	char *path_from_current_dir;
	int max_filename_length;
	struct stat element_stat;
	max_filename_length = 0;
	element_name = NULL;

	/* Open the directory saved in openfile->filename. */
	current_dir = opendir(openfile->filename);

	/*Get the name of file/folder and store it in element. */
	while((element = readdir(current_dir)) != NULL) {

		/* Store the path to 'element' in path_from_current_dir. */
		path_from_current_dir = (char*)malloc(sizeof(char) * (sizeof(filename) + sizeof(element->d_name) + 2));
		strcpy(path_from_current_dir, filename);
		strcat(path_from_current_dir, "/");
		strcat(path_from_current_dir, element->d_name);

		/* Get information about 'element'. */
		if(stat(path_from_current_dir, &element_stat))
			return;

		/* If element is a directory, append a '/' at its end. */
		if(!(S_ISREG(element_stat.st_mode))) {
			element_name = (char *)malloc(sizeof(char) * (sizeof(element->d_name) + 2));
			strcpy(element_name, element->d_name);
			strcat(element_name, "/");
		} else {
			element_name = (char *)malloc(sizeof(char) * (sizeof(element->d_name) + 1));
			strcpy(element_name, element->d_name);
		}

		/* Add an element to a new line on the buffer */
		add_element(element_name);

		/* Find the length of longest file/folder. */
		if(max_filename_length > strlen(element_name))
			max_filename_length = strlen(element_name);

		/* Free the used variables. */
		free(element_name);
		free(path_from_current_dir);
	}

	/* Highlight the first element */
	openfile->mark = openfile->fileage;
	openfile->mark_x = openfile->current_x + strlen(openfile->fileage->data);
...
/* This function adds the data to be added to the current line(node) in the list of lines,
 * increments the totsize of the file, sets the line_no of the current line correctly. It
 * handles various cases to be considered while adding to a list. */
void add_element(char *elem) {

	/* If the first line is NULL or blank, add the data to first line. */
	if(openfile->fileage->data == NULL || !strcmp(openfile->fileage->data,"")) {
		openfile->fileage->data = (char *)malloc(sizeof(char) * (strlen(elem) + 1));
		strcpy(openfile->fileage->data, elem); 
		openfile->fileage->next = NULL;
		openfile->fileage->prev = NULL;
		openfile->totsize = 0;
		return;
	}

	/* If the data is to be added to a non-first line. */
	filestruct *looper;
	filestruct *temp = make_new_node(NULL);
	temp->data = mallocstrcpy(temp->data, elem);
	looper = openfile->fileage;
	openfile->totsize += strlen(elem) + 1;

	/* If the data is to be added to the second line. */
	if(looper->next == NULL) {
		looper->next = temp;
		temp->prev = looper;
		temp->next = NULL;
		openfile->filebot = temp;
		temp->lineno = temp->prev->lineno + 1;
		return;
	}

	/* Increment looper until it points to the line to which data is to be added. */
...
/* Sorts the contents in the buffer i.e. the files and folders in the order of
 * Current Directory first, followed by Parent Directory and then Directories 
 * and atlast Files , each sorted alphabetically. This sorting takes place by 
 * an algorithm in which we find the largest element and swap its data with 
 * the the data of the first unsorted element. */
void sort_contents() {
	int length = openfile->filebot->lineno;
	int i, j;
	filestruct *swapper, *start, *looper;

	/* Loop till all the elements are not in sorted order. */
	for(i = 0; i < length; i++) {

		/* Increment looper and start pointers to point to the first unsorted position. */
		j = i;
		looper = openfile->fileage;
		while(j--) {
			looper = looper->next;
		...
		/* Find the most approprate element by incrementing looper till it reaches the last element. */
		while(looper != NULL) {
			if(!strcmp(looper->data, "./")) {
				swapper = looper;
				break;
			} else if(!strcmp(looper->data, "../")) {
				swapper = looper;
				break;
			} else if(dircmp(swapper->data, looper->data) > 0){
				swapper = looper;
			}
			looper = looper->next;
		}

		/* Swap the data of the lines pointed by start and the appropriate element. */
		swap_data(swapper, start);
	}
}

/* Swaps the data of filestruct 'a' and filestruct 'b' by using a temp variable. */
void swap_data(filestruct *a, filestruct *b) {
	if(!strcmp(a->data, b->data))
		return;
	char *temp;
	temp = NULL;
	temp = mallocstrcpy(temp, a->data);
	a->data  = mallocstrcpy(a->data, b->data);
	b->data  = mallocstrcpy(b->data, temp);
}

/* Function to sort the elements in the required manner. Returns
 * negative value if a is greater than b and returns positive 
 * value if b is greater than a. */
int dircmp(char *a, char *b) {
	int aisdir, bisdir;

	/* Check if a is directory */
	aisdir = isdir(a); 

	/* Check if b is directory */
	bisdir = isdir(b);

	/* If a is directory and b is not a directory then return a > b. */
	if(aisdir && !(bisdir))
		return -1; 
	/* If b is directory and a is not a directory then return b > a. */
	if(!(aisdir) && bisdir)
		return 1; 
	/* If both are directories or both aren't directories then return case insensitive str comparision. */
	return strcasecmp(a, b);	
}

/* Check if the string 'a' is the name of a directory 
 * by checking whether it ends with a '/'. */
int isdir(char *a) {
	if(a[strlen(a) - 1] == '/') 
		return 1;
	return 0;
...
/* Initialize the path_list dir_browser. */
void path_list_init() {
	dir_browser = (path_list*)malloc(sizeof(path_list));
	dir_browser->head = NULL;
	dir_browser->current = NULL;
}
